...
for(species in sp_list){ 
  ...
  load( paste0( 'data/temp_data/', paste0( species, '_survival_weekly_ClimWin.rda')))
  ...
  survival$VWC_best <- temp_ClimWin_fit[[2]]$BestModelData$`log(climate)`
  
  m_baseline <- glmer( survives ~ 1 + (1|group_year) + VWC_best, data = survival, family = 'binomial')
  
  survivalWin_TEMP <- slidingwin(xvar = list(TMAX = daily_weather$tmax_ts, 
  ...
  survivalWin_GDD <- slidingwin(xvar = list(GDD = daily_weather$gdd + 0.00001),
                              cdate = daily_weather$date_reformat,
                              bdate = survival$date,
                              baseline = m_baseline,
                              cinterval = "week",
  ...
  survivalWin_merged <- merge_results(survivalWin_TEMP, survivalWin_GDD)
  
  out_obj_name <- paste(species, 'survival', 'weekly_ClimWin_VWC_cov', sep = '_') # rename climwin results object
  ...
  save(list = out_obj_name, 
       file = paste0( "data/temp_data/", species, "_survival_weekly_ClimWin_VWC_cov.rda"))
  
  rm(dat, survival)
  rm(list = ls(pattern = species)) 
  rm(list = ls(pattern = "survival")) # remove to clear memory 
