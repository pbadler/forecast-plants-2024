...
parameters{
  // for training data model  
	real<lower=0> sigma;            // centrality of skew normal 
...
model{
  ...
  // Likelihood
  target += normal_lpdf(Y_obs | mu_obs, sigma );
  target += normal_lcdf(U | mu_cens, sigma);  // integrate out the censored observations 
}
generated quantities{ 
  ...
  for(i in 1:N){ 
    ...
    if(Y[i] <= U){ 
      log_lik[i] = normal_lcdf(U | mu[i], sigma);
    }
  }
