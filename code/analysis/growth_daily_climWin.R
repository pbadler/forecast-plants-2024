...
for(species in sp_list){ 
  ...
  write_rds( m_baseline, paste0( 'output/growth_models/', species, '_growth_baseline_', model_type, '.rds'))
...
#   # no climate x size 
# 
#   growth <- prep_growth_for_climWin(species, last_year = last_year, quad_info = quad_info, size_cutoff = -1)
#   
#   ##baseline model for growth/size
#   # m_baseline <- lmer( area ~ 1 + area0 + W.intra + (1|year/Group),
#   #                      data = growth,
#   #                      REML = F,
#   #                      control = control_lmer)
#   # model_type <- "mer"
#   
#   m_baseline <- lm( area ~ 1 + area0 + W.intra, data = growth)
#   model_type <- "lm"
#   write_csv( growth, paste0( 'data/temp/', species, '_ClimWin_Growth.csv'))
#   write_rds( m_baseline, paste0( 'data/temp/', species, '_ClimWin_Growth_Baseline_no_intxn_', model_type, '.rds'))
#   
#   growthWin <- slidingwin(xvar = list(TMAX = daily_weather$TMAX, 
#                                       TAVG = daily_weather$TAVG,
#                                       TMIN = daily_weather$TMIN, 
#                                       VWC = daily_weather$VWC),
#                           cdate = daily_weather$date_reformat,
#                           bdate = growth$date_reformat,
#                           baseline = m_baseline,
#                           cinterval = "week",
...
#   
#   save(list = out_obj_name, 
#        file = paste0( "data/temp/", species, "_growth_no_intxn_", model_type, "_monthly_ClimWin.rda"))
#   
# }
